...
const (
	bitgetAPIUrl = "https://api.bitget.com/api/v2/spot/market/tickers?symbol=ABUSDT"
)
...
func NewPriceGetterBitget() *PriceGetterBitget {
	...
	httpClient.SetRetryWaitTime(time.Second / 2)
...
func (pg *PriceGetterBitget) GetLatest2() (price decimal.Decimal, timestampSec int64, err error) {
	resp, err := http.Get("https://api.bitget.com/api/v2/spot/market/tickers?symbol=ABUSDT")
	if err != nil {
		return decimal.Zero, 0, err
	}
